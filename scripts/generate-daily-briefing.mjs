...
try {
  previousInsights = String(JSON.parse(await readFile(outputFile, "utf8")).insights || "").trim();
} catch {
  previousInsights = "";
}
...
const editorialQuestion = [
  ...
  "Two mix requirements, both binding. At least two bullets must be about a venue other than Kalshi or must compare venues against each other -- Kalshi is the deepest tape here, not the subject of the briefing, and the venue figures alone always support a cross-venue bullet, so this never requires inventing significance. And one bullet must carry Kalshi data from beyond plain volume. ROTATE THROUGH THESE rather than settling on one -- each names the table that answers it: overall taker P&L and how it splits against parlay P&L (taker_pnl_daily, parlay_pnl_unified_daily); longshot or lottery-ticket parlay activity, including whether betting shifted toward lower-priced, longer-odds contracts (parlay_lottery_daily, parlay_lottery_summary); which individual games or events pulled the parlay money (parlay_top_games_by_volume, parlay_popular_daily, with event_titles for readable names); an unusually large single trade or a shift in trade sizes (extreme_trades_daily, trade_size_daily, parlay_trade_size_daily); the sports, non-sports and parlay split (daily_sports_vs_nonsports); taker-side dollar volume against contract volume, i.e. whether the money moved with the count (taker_notional_daily); fee revenue and the blended fee rate (daily_overall, category_daily); or house edge by leg count (parlay_house_edge_by_legs). Nothing else in this industry is measured to that depth. Normally it rides in the same bullet as the Kalshi headline volume, after the number -- lead with what traded, then say what the money did. Give it a bullet of its own only when that figure is genuinely notable on its own terms. That bullet must cover the same recent window as the rest of the briefing: a figure for the latest reported day or the past week, set against a recent baseline. Never reach for an all-time or months-old cumulative total -- this card reports what changed, not what has accumulated. A briefing that is entirely Kalshi has failed; so has one that only ever counts contracts.",
  ...
  "BE PRECISE ABOUT LOTTERY-TICKET PARLAYS. parlay_lottery_daily is an aggregate slice of parlays with at least eight legs trading below 2 cents. Its volume is contracts, NOT a count of tickets; stakes is the total money yes-takers put in; trades is the number of trades; and n_tickers is the number of distinct parlay markets active that day. More contract volume alongside less total stake does not show that the same tickets got cheaper, nor by itself that users bought more tickets. A lower stakes-per-contract ratio shows that the aggregate mix shifted toward lower-priced, longer-odds contracts. Say that plainly -- for example, \"betting shifted toward longer odds\" -- and reserve \"got cheaper\" for a like-for-like query showing that the same parlay tickers fell in price over time.",
...
const KALSHI_DEPTH = /\b(fees?|revenue|p&l|pnl|profit|loss(es)?|parlays?|sports|non-sports|settled?|settlement|accuracy|takers?|makers?|mix|categor(y|ies)|trade size|lottery|longshots?|stakes?|house edge|fee rate)\b/i;
...
const ESCALATION = " Be concrete this time. Pick one table from the rotation -- taker_pnl_daily, parlay_lottery_daily, parlay_top_games_by_volume, extreme_trades_daily, daily_sports_vs_nonsports or taker_notional_daily -- query it for Kalshi's own latest complete date, compare that figure against its average over the prior thirty reported days, and put the result in the Kalshi bullet after the headline volume.";
