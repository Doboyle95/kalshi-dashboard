...
const editorialQuestion = [
  ...
  "Then run only the supporting context queries needed to find genuinely interesting changes. Cross-venue evidence to prefer: competitor_daily contracts and fees for the non-Kalshi platforms, prophetx_daily, cme_daily, Novig, parlay adoption at venues that identify multi-leg products, and settled-outcome accuracy where a venue has enough settled contracts. Kalshi goes far deeper than volume, and that depth is the most valuable material here: product mix, sports versus non-sports, parlay share, fee revenue, taker-side volume, taker P&L, settlement accuracy, unusually large individual trades, and extremely long-odds parlays. You MUST run at least one new supporting query over one of the named Kalshi depth tables below. A general phrase such as parlays, sports, product mix, or August share does not satisfy this requirement by itself, and neither does a figure recalled from static context.",
  "Two mix requirements, both binding. At least two bullets must be about a venue other than Kalshi or must compare venues against each other -- Kalshi has the most detailed trade records here, not the subject of the briefing, and the venue figures alone always support a cross-venue bullet, so this never requires inventing significance. And one bullet must carry a specific Kalshi figure from a supporting query beyond plain volume. ROTATE THROUGH THESE rather than settling on one -- each names the table that answers it: overall taker P&L and how it splits against parlay P&L (taker_pnl_daily, parlay_pnl_unified_daily); which individual games or events pulled the parlay money (parlay_top_games_by_volume, parlay_popular_daily, with event_titles for readable names); an unusually large single trade or a shift in trade sizes (extreme_trades_daily, trade_size_daily, parlay_trade_size_daily); the sports, non-sports and parlay split (daily_sports_vs_nonsports), including the mix of moneylines, spreads, totals, game props and other sports market types (sports_market_type_daily); taker-side dollar volume against contract volume, i.e. whether the money moved with the count (taker_notional_daily); fee revenue and the blended fee rate (daily_overall, category_daily); house edge by leg count (parlay_house_edge_by_legs); or the recent activity and hit rate of extremely long-odds parlays (parlay_lottery_daily, optionally paired with parlay_lottery_summary). The last pair uses an internal definition: at least eight legs and that day's yes-side average price below 2 cents. If you use it, the first reference must say either \"parlays with at least eight legs trading below 2 cents\" or \"extremely long-odds parlays,\" making clear it is a subset rather than parlays generally. Never write lottery ticket, parlay-lottery, lottery volume, longshot, or longer-odds. Nothing else in this industry is measured to that depth. Normally the deep figure rides in the same bullet as Kalshi's headline volume, after the number -- lead with what traded, then say what the money did. Give it a bullet of its own only when that figure is genuinely notable on its own terms. That bullet must cover the same recent window as the rest of the briefing: a figure for the latest reported day or the past week, set against a recent baseline. An August-to-date share alone is not enough. Never reach for an all-time or months-old cumulative total -- this card reports what changed, not what has accumulated. A briefing that is entirely Kalshi has failed; so has one that only ever counts contracts.",
  ...
  previousInsights
    ...
    : "",
  "Return three to five bullets, no more than 150 words in total. One sentence per bullet, two at the very most. Lead each with a short bold phrase naming the finding, then give the numbers against a sensible recent baseline. Round large figures the way a reader would say them -- 2.46B contracts, 346.8M, $16.0M -- and when a comparison figure is an average, call it one, as in \"a daily average of 1.23B over the past month\". Bold only that opening phrase -- leave the figures themselves unbolded. The bold phrase must say WHY the item is here: a reader scanning only those phrases should still know what changed or was surprising. An ordinary standing is not a finding. Do not write \"Polymarket led the challengers\", \"X was the largest competitor\", \"X held second place\" or similar unless the rank itself changed or the gap became genuinely remarkable; when growth is the story, put the growth in the opening phrase instead. Do not force a topic when nothing notable happened there. Kalshi's headline contract count is rarely the most interesting figure available to you; do not open with it unless the move is genuinely unusual. Before you finish, check the draft against the two mix requirements above and rewrite a bullet if either is unmet.",
  "SIGNIFICANCE IS NOT PERCENTAGE CHANGE, and ranking by percentage is the most common way this briefing goes wrong. Every row carries approx_market_share_pct, that venue's rough share of the ten venues together. Call it market share in plain words -- \"about a 1.3% market share\", \"roughly 7% of the market\" -- which is the term the rest of the site uses. Never write \"measured venue volume\", \"measured volume\", or any similar construction; it does not tell a reader what is being measured. The same percentage means very different things at different venues: the smaller and mid-sized ones routinely swing 30-40% in a day, while the largest rarely move more than 25%, so a big percentage is often an ordinary day. Weigh two separate things before calling any move notable -- whether it is unusual for THAT venue, judged against its own recent range, and whether it is large enough to matter at industry scale. One query over that venue's last thirty days settles the first. These rules decide which venue MOVES are worth reporting; they do not replace the required shape stated at the top. The Kalshi bullet from beyond plain volume is still required -- but a bullet about Kalshi's SIZE or share is not that bullet, and does not satisfy it. Kalshi's scale does not carry over to a slice of it, and slices come in two kinds with different bars. A CATEGORY -- economics, elections, politics, commodities, weather and the like -- is judged by size: one holding under about 1% of Kalshi's day is not news on growth alone, however large the percentage, so leave it out. Outside sports and crypto that is nearly every category on nearly every day. A SINGLE market, event, game or trade is judged by whether it stands out, and can earn a clause at a far smaller volume: the biggest market in the biggest category, or an unusually large trade, is reasonable to name. Size either kind against Kalshi's total for the day.",
...
function draftFaults(result) {
  ...
  return [
    ...(/\btape\b/i.test(text) ? ["replace 'tape' with plain, context-specific wording"] : []),
    ...formatFaults(text),
    ...mixFaults(text),
    ...kalshiDepthEvidenceFaults(sqls),
    ...kalshiRecordDayFaults(text, kalshiStanding),
    ...kalshiCategoryFaults(text, kalshiRow ? anchorVolume(kalshiRow) : null),
    ...wordingFaults(text, sqls)
  ];
}
...
const ESCALATION = " Be concrete this time. Pick one table from the rotation -- taker_pnl_daily, parlay_top_games_by_volume, extreme_trades_daily, daily_sports_vs_nonsports, taker_notional_daily or parlay_lottery_daily -- query it for Kalshi's own latest complete date, compare that figure against its daily average over the previous thirty days, and put the result in the Kalshi bullet after the headline volume. Do not reuse an August-to-date parlay share. If parlay_lottery_daily is strongest, call the subset parlays with at least eight legs trading below 2 cents or extremely long-odds parlays; never expose the internal table label.";
