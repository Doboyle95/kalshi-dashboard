...
<div class="page-hero" data-accent="kalshi">
  ...
  <p class="page-lead">ProphetX (pink) and Novig (indigo) are sports-native peer-to-peer exchanges, and both are now CFTC-designated contract markets — ProphetX approved in June 2026, Novig trading on its own Ludlow Exchange, designated 2026-06-16 — and both publish a full public trade tape, so both are on the chart. Leaving them off did not make this page partial, it made it <em>wrong</em>: over its ${leadStats.px.days} complete days ProphetX has traded <strong>${fmtLead(leadStats.px.contracts)} contracts</strong>, ${leadStats.px.vsUnderdog}&times; Underdog Exchange and ${leadStats.px.vsDkex}&times; DKeX across that same window, and Novig traded <strong>${fmtLead(leadStats.novig.contracts)}</strong> in its first ${leadStats.novig.days} days, ${leadStats.novig.vsUnderdog}&times; Underdog and ${leadStats.novig.vsDkex}&times; DKeX over those days.${leadStats.rankSentence} <strong>Both venues publish a fee schedule</strong>, and neither can be drawn on the fee charts below, for two different reasons: ProphetX charges a percentage of a trader’s net gains rather than a rate per contract, and Novig charges a rate per contract but only on some of its trade types, with nothing in its tape to say which trades those were.</p>
</div>
...
<details class="surface-card compact-details">
  ...
  <p><strong>Two venues carry no fee number at all — and it is not for want of a published schedule. Both of them have one.</strong> Their files carry no fee column, so every fee chart on this page leaves them out and <em>names</em> them underneath rather than drawing them on the zero line. Neither reason is a disclosure we are waiting on, and the two are not the same reason. <strong>ProphetX</strong> publishes a 2% trading fee on <em>net gains per market</em> for straight trades and 0% for parlays (schedule v1.0, last updated 10 June 2026, so it covers every day of the tape drawn here, which opens 2026-06-16). That is a commission on a trader’s realized profit inside a single market, so computing it needs per-user identity to net one user’s wins against their losses — and this tape is anonymous. Applying 2% to volume instead is a different and much larger number, since net gains are a small fraction of traded value, and the parlay exemption is material: parlays are 11.7% of ProphetX volume across its tape through 2026-08-13. So ProphetX has no fee series on this data and will not get one. <strong>Novig</strong> publishes a per-contract schedule instead: nothing at all on pre-game straight trades, 0.03 &times; P &times; (1&minus;P) per contract charged to the taker on live in-game straights, 0.10 &times; P &times; (1&minus;P) on parlays with that fee already inside the quoted price, no fee to any maker on anything, and no settlement fee — winning contracts are paid the full $1.00. The parlay half of that is exactly computable from its tape. The straight half is not computable at all, because nothing in the tape records whether a trade was placed pre-game or in-game, and the two rates are $0 and a parabola. Novig’s realized fee is therefore a range rather than a number, and one line on a chart would state a precision that does not exist. Novig also advertises commission-free trading, and the spread implied by its own tape does measure at zero on <a href="./novig">its page</a> — both things are true at once. There is no spread, and there is a real per-contract commission on two of its three trade types; a spread test cannot see a separate per-contract fee, so neither measurement corrects the other. Absent here means not measured, never free.</p>
  ...
  <p><strong>This page compares how much trades and what it costs, not whether the prices are right.</strong> That question &mdash; do a venue's prices actually predict outcomes &mdash; is on the <a href="./calibration-venues">cross-venue calibration page</a>, which puts Kalshi, Polymarket US, ForecastEx, DKeX and ProphetX on one axis with event-clustered error bars — ProphetX joined them on 2026-08-14, once its settlement column was shown to score single markets cleanly. Four venues drawn on this page are named absent there with the reason stated: Rothera, Underdog Exchange, Crypto.com/Nadex and CME. Novig is the one venue here that page does not mention at all.</p>
</details>
...
const platforms = [
  ...
  {
    // ProphetX DOES publish a fee schedule: 2% of NET GAINS PER MARKET on straight trades and
    // 0% on parlays (v1.0, last updated 2026-06-10, so it covers this whole tape). It is
    // UNCOMPUTABLE here, not unpublished. A commission on realized profit inside one market
    // needs per-user identity to net a user's wins against their losses, and this tape is
    // anonymous; 2% of VOLUME is a different and far larger number, because net gains are a
    // small fraction of traded value. The 0% parlay carve-out is material too -- parlays are
    // 11.7%
    // of ProphetX volume. So fees and revenue stay null on purpose and this venue is ABSENT
    // from every fee chart and named underneath it, never drawn on the zero line: `+null` is 0,
    // and a 0.000c per-contract rate would be a claim about price we cannot make. tradedValue is
    // null for the same reason -- the file carries none, so it cannot appear in basis points.
  ...
  },
  {
    ...
    // Novig is NOT commission-free and DOES publish a schedule: the taker pays 0.03*P*(1-P) per
    // contract on LIVE straights and 0.10*P*(1-P) on parlays (already inside the quoted price),
    // pre-game straights are free to both sides, every maker pays nothing, and winners are paid
    // the full $1.00. Only the parlay half is measurable from the tape, and it is measurable
    // EXACTLY -- the parlay price printed is all-in, so the fee is recovered by inverting
    // quoted = fair + 0.10*fair*(1-fair). The straight half is not measurable at all: nothing in
    // the tape separates pre-game from live and those two rates are $0 and a parabola, so the
    // venue total is a BOUND rather than a point. That is why these cells stay empty even though
    // the schedule is now known -- a single line cannot state a range honestly.
...
<p style="font-size:0.82em;color:#999;margin-top:0.5rem">Per-side fee divided by the day's contracts (or by the day's traded value, in basis points). This is the realized rate a venue actually collected, not its posted schedule, so it moves with the venue's price mix. <strong>Cents per contract is the honest cross-venue unit here.</strong> Basis points of traded value flatters flat-rate venues at high prices and punishes them at low ones — DKeX's published $0.01 per contract works out at about 260bp of traded value purely because its average traded price is near 47c, which is arithmetic about price mix, not a pricing difference in spirit. Basis points also need a traded-value denominator, which we hold for only some venues; the note under the chart names the ones that cannot appear. All the caveats under the volume chart apply here too — Rothera's daily-price proxy and assumed participant mix, Polymarket US's inferred regime dates, and the fact that unobservable volume-tier rebates make every one of these an upper bound. ForecastEx's count is matched pairs, which is one contract per side, so its per-contract rate is on the same footing as the rest. <strong>ProphetX and Novig do not appear on this chart in any window, and both of them do publish a fee schedule</strong> — what is missing is our numerator, not their disclosure. ProphetX charges 2% of a trader’s net gains per market on straight trades and 0% on parlays. That is a commission on realized profit, not a rate per contract: netting one user’s wins against their losses inside a single market needs per-user identity, our tape is anonymous, and its parlays — 11.7% of its volume — are exempt outright. On this data that absence is permanent. Novig’s schedule is per contract and partly computable: $0 on pre-game straight trades, 0.03 &times; P &times; (1&minus;P) to the taker on live straights, 0.10 &times; P &times; (1&minus;P) on parlays. Its parlay fees can be measured exactly; its straight fees cannot be measured at all, because the tape carries no pre-game/live flag and the two rates are zero and a parabola, so its realized rate is a range rather than a point and a single line here would state a precision that does not exist. The note above names them alongside any venue that is merely missing a number in the selected window; in every case absent means unmeasured, not cheap.</p>
...
```js
{
  ...
  const curves = [
    ...
    {name: "Novig — parlay",           f: p => 10 * p * (1 - p), curve: "monotone-x", dash: null,
                                        color: "#6366F1"},
    {name: "Novig — live straight",    f: p => 3 * p * (1 - p), curve: "monotone-x", dash: "5,4",
                                        color: "#A5B4FC"},
  ...
  // The legend on THIS chart is the curves, not every venue on the page. ProphetX publishes a
  // real schedule, but it is 2% of NET GAINS PER MARKET -- a commission on realized profit, which
  // is not a function of contract price at all, so there is no curve to put on this axis and it
  // gets no swatch. A legend swatch with no line next to it reads as a venue drawn at zero, the
  // exact false claim this page's fee guards exist to prevent; it is named in the caption instead.
  // Novig IS drawn, because its schedule IS a per-contract parabola like the others -- but it
  // takes two lines, not one: 0.10 on parlays and 0.03 on LIVE straights. Its third regime,
  // pre-game straights, is free to both sides and is stated in the caption rather than drawn,
  // because a flat zero line here is indistinguishable from the "not measured" absence the rest
  // of this page works to avoid. Novig's two curves carry an explicit colour because they are not
  // in colorDomain under these names; indexOf would return -1 and hand Plot an undefined range.
  const curveNames  = curves.map(c => c.name);
  const curveColors = curves.map(c => c.color ?? colorRange[colorDomain.indexOf(c.name)]);
  ...
  display(Plot.plot({
    ...
    y: {label: "Fee per contract, one side (cents)", grid: true, domain: [0, 2.6], tickFormat: d => d.toFixed(1) + "c"},
...
<p style="font-size:0.82em;color:#999;margin-top:0.5rem">Per-side exchange fee per contract at each price, from each venue's published schedule. Four venues charge the same parabola with a different coefficient: Kalshi and Underdog Exchange both use 0.07 (the lines coincide exactly — Underdog is dashed so both are visible — but Underdog charges it to <em>both</em> sides, so it collects roughly twice per matched trade), Polymarket US 0.06 on the taker with a 0.0125 rebate back to the maker, Rothera 0.02 for retail. Three are flat: Crypto.com/Nadex $0.02, ForecastEx $0.01 (per side since 2026-05-01; before that a single penny per matched pair), CME $0.01. DKeX is a step with three levels, not two: $0.01 across the body of the book, $0.0085 at 2c, and half a cent at 1c and 99c — and the 2c step has no counterpart at 98c, so the ladder is not symmetric about the middle of the book. <strong>The crossings are the point:</strong> CME and DKeX are cheaper than Kalshi through the middle of the book and dearer at the tails, so no single ranking holds everywhere. Rounding is omitted; Rothera's professional tier is 6x the retail line drawn here and its participant mix is unobservable; broker commissions are excluded throughout. <strong>Novig has two lines here now, and they are the extremes of the chart.</strong> It is not commission-free: its taker pays 0.10 &times; P &times; (1&minus;P) per contract on parlays, the steepest parabola drawn — above Kalshi’s 0.07, peaking at 2.5c at even money, and quoted all-in, so the fee is already inside the price a trader sees — and 0.03 &times; P &times; (1&minus;P) on <em>live</em> straight trades, which is cheaper than every venue here except Rothera’s retail tier. Its third regime cannot be drawn: pre-game straight trades are free to both sides, and a flat zero line is exactly the “absent means free” reading the fee guards on this page exist to prevent. Every maker on Novig pays nothing on everything; a maker credit worth a share of the taker fee — reported as up to half, and not published as an exact rate — is paid back on qualifying live straights but never on parlays, so its parlay revenue is the full taker fee. Winners are paid the full $1.00 with no settlement fee. <strong>ProphetX still has no line, and not because it publishes nothing:</strong> its schedule is 2% of a trader’s net gains per market on straights and 0% on parlays, a commission on realized profit rather than a price-dependent rate per contract, so there is nothing to plot on this axis and it is named here rather than given a legend swatch with nothing under it.</p>
...
const OI_ABSENT = new Map([
  ...
  ["ProphetX",                   "publishes an open-interest column only inside its daily bulletin, and that column is stamped rather than measured: one aggregate value is repeated across thousands of multi-event rows, so on the median session 100% of the parlay open-interest total sits in values shared by more than one contract, the largest single identical-value group covers 1,628 contracts, and parlay rows carry a median 98.4% of the venue's whole open-interest total. The same stamping is what makes the bulletin's volume column unusable: deduplicated, and compared only on the contracts the trade tape also lists, it agrees with the tape within 2% on 2 of the venue's 60 sessions through 2026-08-14, which is why volume here comes from the tape instead, and the tape carries no open interest. This site does read the bulletin — its settlement_price column supplies all 63,915 single-market resolutions behind the ProphetX calibration — and the stamping is exactly what spares that column: on single markets a median 0.28% of the open-interest total sits in a repeated value, against 100% on the parlay side, and open interest is 0 on 97.3% of those 63,915 resolved singles. A venue-wide book has to include the parlays, so there is still no ProphetX line here"]
]);
