---
title: Polymarket US · Trading behavior
---

<div class="page-hero" data-accent="polymarket">
  <div class="page-eyebrow">Polymarket US</div>
  <h1>How it trades</h1>
  <p class="page-lead">Trade sizes, where on the probability axis the volume sits, and the biggest individual prints Polymarket US has published.</p>
</div>
...
const size = sizeAll.filter(d => d.platform === "Polymarket US" && d.segment_type === "All");
...
const openTrade = attachTradeInspector({source: "polymarket-trades", venue: VENUE, largeTrades: lt, metrics: metricsFor(lt, VENUE)});
...
const vapBins = Array.from(
  d3.rollup(
    vapRaw.filter(d => (d.period == null || d.period === "all") && (d.group == null || d.group === "LEG_PRICE")),
...
<div class="instruction-line" style="border-left-color:var(--theme-foreground-muted)">Where traders choose to put money on the probability axis. This is behaviour, not accuracy — see <a href="./polymarket-calibration">Outcomes</a> for whether those prices came true.</div>
...
<div class="instruction-line" style="border-left-color:var(--theme-foreground-muted)">See also: <a href="./polymarket">Polymarket US &middot; Activity</a>, and <a href="./trade-size">Trading behavior across venues</a> for the comparison.</div>
