...
const lbMarkets = normalizeLeaderboard("polymarket", lbRows);
display(marketLeaderboard({
  hashPrefix: "pmlb",
  venues: [{spec: LB_VENUES.polymarket, rows: lbMarkets}],
  onMarketSelect: attachMarketInspector({source: "polymarket-markets", rows: lbMarkets})
}));
```
