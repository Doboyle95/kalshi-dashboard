...
<details class="surface-card compact-details">
  ...
  <p>Volume is contract count (one settled contract per unit), summed by day. Categories come from each market's product prefix (e.g. <code>MWC*</code> = World Cup soccer, <code>MLB*</code> = baseball, <code>EC*</code> = economics); the sports split sums sport prefixes against everything else. The exports carry no fee field, so fees are computed per fill from Rothera's trade records against its published schedule &mdash; see <a href="./rothera-economics">Economics</a>, which is why that series starts later than this one. Because <em>this</em> page is end-of-day market data rather than trade prints, it is best for scale and category mix; <a href="./rothera-behavior">Trading behavior</a> covers microstructure over the shorter period covered by trade records.</p>
</details>
...
<div class="instruction-line" style="border-left-color:var(--theme-foreground-muted)">Measured 2026-09-01. The numbers above move as more trades are recorded; re-run before citing them.</div>
