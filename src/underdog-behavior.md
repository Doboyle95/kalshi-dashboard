---
title: Underdog Exchange · Trading behavior
---

<div class="page-hero" data-accent="underdog">
  <div class="page-eyebrow">Underdog Exchange</div>
  <h1>How it trades</h1>
  <p class="page-lead">Trade sizes, where on the probability axis the volume sits, and the biggest individual prints Underdog Exchange has published.</p>
</div>
...
const vapRaw = await DataAttachment("data/underdog_volume_at_price.csv").csv({typed: true});
```

```js
const ACCENT = "var(--accent-underdog)";
const VENUE = "Underdog Exchange";
...
const size = sizeAll.filter(d => d.platform === "Underdog Exchange" && d.segment_type === "All");
...
const openTrade = attachTradeInspector({source: "underdog-trades", venue: VENUE, largeTrades: lt, metrics: metricsFor(lt, VENUE)});
...
const vapBins = Array.from(
  d3.rollup(
    vapRaw.filter(d => d.group == null || d.group === "SINGLE"),
...
<div class="instruction-line" style="border-left-color:var(--theme-foreground-muted)">Where traders choose to put money on the probability axis. This is behaviour, not accuracy: it says nothing about whether those prices came true.</div>
...
<div class="instruction-line" style="border-left-color:var(--theme-foreground-muted)">See also: <a href="./underdog">Underdog Exchange &middot; Activity</a>, and <a href="./trade-size">Trading behavior across venues</a> for the comparison.</div>
