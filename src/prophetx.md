...
<details class="surface-card compact-details">
  ...
  <p><strong>Whose calibration this is.</strong> ProphetX publishes <strong>no aggressor flag</strong>: the tape records that a trade happened at a price, not who crossed the spread. This is therefore <em>not</em> a taker curve and must not be described as one. It is the <strong>priced side</strong> &mdash; the second-named side after the &ldquo;@&rdquo;, i.e. the home side, which is the only side the venue quotes. The counterparty holds the mirror image: it paid ${(100 * (1 - calPaid)).toFixed(2)}&cent; and won ${(100 * (1 - calWin)).toFixed(2)}% of contracts, the same ${Math.abs(100 * calTilt).toFixed(2)}&cent; the other way. The Kalshi series on <a href="./compare-accuracy">Accuracy &amp; Outcomes</a> bins the taker's own side; this bins a fixed side of the market, so the two are not the same statistic and should not be differenced.</p>
