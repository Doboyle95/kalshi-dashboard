...
<details class="surface-card compact-details">
  ...
  <p><strong>No prices anywhere on this page, deliberately.</strong> Other venue pages here carry an average price and a dollar figure; ProphetX gets neither — not because its price is meaningless, but because it cannot yet be attached to an outcome. The price <em>is</em> real: tested against 1,088 settled markets it calibrates monotonically across all ten price bins, and the higher-priced side wins 62% of the time. What is missing is the mapping from a print to the side it backs. A ProphetX contract id is a tradeable instrument at a price level rather than a side — one market line carries several ids, each showing <em>both</em> selection labels (one baseball total has three ids at 0.63, 0.53 and 0.48, every one of them labelled both Over and Under), and 1,248 of 3,247 two-id markets settle the <em>same</em> way rather than opposite, which true opposite sides could not do.</p>
  <p><strong>Guessing the side anyway</strong> produces a curve that is centred and monotone but badly flattened: 5¢ contracts appear to win 25% of the time against a true ~5%, while 85¢ contracts win 79% against ~85%. Published, that would read as “ProphetX longshots are wildly underpriced” when it is an artefact of the join, so calibration, P&amp;L and volume-at-price stay absent until the side convention is established from ProphetX’s own specification. <strong>Contract counts are unaffected</strong> — they are a plain quantity sum, and they match the venue’s own bulletin exactly on the one session where the bulletin is sane.</p>
