...
<div class="instruction-line" style="border-left-color:var(--theme-foreground-muted)"><span style="color:${PX}">&#9679;</span> clears 2 fixture-clustered SE &nbsp; <span style="color:var(--theme-foreground-muted)">&#9675;</span> indistinguishable from calibrated &nbsp;&middot;&nbsp; bars are &plusmn;2 fixture-clustered SE &nbsp;&middot;&nbsp; dot area &prop; fixtures in the band &nbsp;&middot;&nbsp; <strong>${calClearPaid} of ${calBins.length}</strong> bands clear that bar. ${calClearPaid === 0 ? html`<strong>Every dot is hollow &mdash; that is the finding, not a missing layer.</strong>` : ""}</div>
