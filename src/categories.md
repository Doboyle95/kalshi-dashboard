...
const parlayCorrByTicker = await DataAttachment("data/parlay_corr_by_ticker_daily.csv").csv({typed: true});
...
import {
  ...
  ALL_TEAMS, GOLF_PLAYERS, getTeamsForMarket, parseGame, PGA_EVENTS, toRoman, STATE_NAMES,
  FUTURES_YEAR_EVENTS, parseTicker, MKT_NAME_FORCE, MKT_NAME_OVERRIDES, WINNER_OVERRIDES,
  WINNER_BY_MARKET, fmtWinner, TICKER_VOCAB, tokenizeVocab, humanizeUnresolved, bestName,
  TOP_OUTCOME_NAMES, fmtStrike, getSportDisplayCategory
} from "./components/ticker-names.js";
```
...
```js
const sportsSplit = await DataAttachment("data/daily_sports_vs_nonsports.csv").csv({typed: true});
```
...
const parlayByType = await DataAttachment("data/parlay_volume_by_type_daily.csv").csv({typed: true});
...
const dailyOverallFees = await DataAttachment("data/daily_overall.csv").csv({typed: true});
```
...
```js
const marketTypeRaw = await DataAttachment("data/sports_market_type_daily.csv").csv({typed: true});
```
...
```js
// CAT_COLORS, team/player dicts, getTeamsForMarket, parseGame, parseTicker,
// MKT_NAME_FORCE/OVERRIDES, WINNER_OVERRIDES/BY_MARKET, fmtWinner, TICKER_VOCAB,
// tokenizeVocab, humanizeUnresolved, bestName, TOP_OUTCOME_NAMES, fmtStrike,
// getSportDisplayCategory now live in ./components/ticker-names.js (imported
// above) so trade-size.md's large-trades tables can reuse the same naming logic.
