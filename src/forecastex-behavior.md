---
title: ForecastEx · Trading behavior
---

<div class="page-hero" data-accent="forecastex">
  <div class="page-eyebrow">ForecastEx</div>
  <h1>How it trades</h1>
  <p class="page-lead">Trade sizes, where on the probability axis the volume sits, and the biggest individual prints ForecastEx has published.</p>
</div>
...
const vapRaw = await DataAttachment("data/volume_at_price_forecastex.csv").csv({typed: true});
```

```js
const ACCENT = "var(--accent-forecastex)";
const VENUE = "ForecastEx";
...
const size = sizeAll.filter(d => d.platform === "ForecastEx" && d.segment_type === "All");
...
const openTrade = attachTradeInspector({source: "forecastex-trades", venue: VENUE, largeTrades: lt, metrics: metricsFor(lt, VENUE)});
...
const vapBins = Array.from(
  d3.rollup(
    vapRaw.filter(d => d.leg == null || d.leg === "yes"),
    rs => ({contracts: d3.sum(rs, d => +d.contracts || 0), dollars: d3.sum(rs, d => +d.dollars || 0)}),
    d => +d.price_bin
  ),
...
<div class="instruction-line" style="border-left-color:var(--theme-foreground-muted)">Where traders choose to put money on the probability axis. This is behaviour, not accuracy — see <a href="./forecastex-outcomes">Outcomes</a> for whether those prices came true.</div>
...
<div class="instruction-line" style="border-left-color:var(--theme-foreground-muted)">See also: <a href="./forecastex">ForecastEx &middot; Activity</a>, and <a href="./trade-size">Trading behavior across venues</a> for the comparison.</div>
